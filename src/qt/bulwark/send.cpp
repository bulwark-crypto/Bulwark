...
SendWidget::SendWidget(BWKGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("BWK");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("zBWK");
    setCssProperty(ui->pushRight, "btn-check-right");

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (BWK) or private coins (zBWK)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("Enter a Bulwark address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 BWK");
...
void SendWidget::onSendClicked(){
    ...
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendPiv ? AskPassphraseDialog::Context::Send_PIV : AskPassphraseDialog::Context::Send_zBWK, true)){
...
bool SendWidget::sendZTelos(QList<SendCoinsRecipient> recipients){
    ...
    if (walletModel->sendZTelos(
        ...
        inform(tr("zBWK transaction sent!"));
        ...
        emit message("zBWK transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isPIV){
        ...
        if (walletModel->getZerocoinBalance() > 0) {
            ...
            inform(tr("You don't have any zBWK in your balance to select."));
        }
    }
}
...
void SendWidget::onPIVSelected(bool _isPIV){
    isPIV = _isPIV;
    setCssProperty(coinIcon, _isPIV ? "coin-icon-piv" : "coin-icon-zbwk");
    refreshView();
