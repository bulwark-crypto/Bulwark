...
PIVX Core v3.0.2 is a recommended, semi-mandatory update for all users. This release contains transaction creation bug fixes for zBWKspends, automint calculation adjustments, and other various updates/fixes.

zBWKspending requires this update.
...
Auto Wallet Backup
---------------------
In addition to the automatic wallet backup that is done at each start of the client, a new automatic backup function has been added that will, by default, create a backup of the wallet file during each zBWKmint operation (zBWKspends which re-mint their change are also included in this). This functionality is controlled by the `-backupzBWK command-line option, which defaults to `1` (enabled, auto-backup).

Users that wish to prevent this behavior (not recommended) can pass `-backupzBWK0` at the command-line when starting the client, or add `backupzBWK0` to their `pivx.conf` file.

zBWKAutomint Calculations
---------------------
A bug in the automint calculations was made apparent on mainnet when block times exceeded expectations, resulting in zBWKmint transactions that were in an unconfirmed state to still be treated as if they had never been minted. This caused automint to effectively mint more than what was intended.

zBWKSpending Fix
---------------------
The size of zBWKspend transactions is knowingly larger than normal transactions, and while this was expected, a much stricter check against the scriptsig size is used for mainnet, causing the transactions to be rejected by the mempool, and thus not being packaged into any blocks.

zBWKTransaction Recovery
---------------------
Due to the aforementioned issue with zBWKspending, users may find that their attempted spends are now conflicted and zBWKbalances are not represented as expected. "Recovery" of these transactions can be done using the following methods:
...
- #279 `e734010` Add -backupzBWKstartup flag. (presstab)
